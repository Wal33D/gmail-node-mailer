...
): Promise<ISendEmailFunctionResponse> {
  const { senderEmail, senderName, recipientEmail, subject, message, attachments } = params;
  try {
    const { encodedContent: encodedSubject } = encodeEmailContent({
      content: subject || '',
      type: EncodingType.Subject,
    });
    const { isHtml } = detectHtml({ content: message });

    const boundary = '----=_NextPart_' + Math.random().toString(36).substring(2, 11);
    ...
    const { encodedContent, isEncoded } = encodeEmailContent({
      content: mimeMessage,
    ...
    return {
      ...
      message:
        gmailResponse.status >= 200 && gmailResponse.status < 300
